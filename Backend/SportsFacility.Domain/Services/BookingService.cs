...
namespace SportsFacility.Domain.Services
{
    public class BookingService : IBookingService
    {
        ...
        public async Task<Booking?> CreateBookingAsync(BookingListDto dto)
        {
            ...
            if (!string.IsNullOrEmpty(dto.ActivityId) && Guid.TryParse(dto.ActivityId, out var facilityGuid))
            {
                facility = await _context.Facilities.FindAsync(facilityGuid);
            }

            if (facility == null)
            {
                facility = await _context.Facilities.FirstOrDefaultAsync(f => f.Name == dto.FacilityName);
                if (facility == null)
                {
                    ...
                    facility = await _context.Facilities.FirstOrDefaultAsync(f => f.Name == facilityName);
                    if (facility == null)
                    {
        ...
        {
            var booking = await _context.Bookings.Include(b => b.User).Include(b => b.Facility).FirstOrDefaultAsync(b => b.Id == id);
            if (booking == null) return false;

            if (!string.IsNullOrEmpty(dto.ActivityId) && Guid.TryParse(dto.ActivityId, out var facilityGuid))
            {
                booking.FacilityId = facilityGuid;
            }
            else if (!string.IsNullOrEmpty(dto.FacilityName))
            {
                var facility = await _context.Facilities.FirstOrDefaultAsync(f => f.Name == dto.FacilityName);
                if (facility != null)
                {
